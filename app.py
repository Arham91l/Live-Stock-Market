...
def plot_anomaly_zscore(anomaly_data, ticker):
    ...
    if z.empty:
        ...
        fig.add_annotation(
            text="No data available for this ticker",
            xref="paper", yref="paper",
            x=0.5, y=0.5, showarrow=False,
            font=dict(color="#7ab3ef", size=13)
        )
        return fig
...
def plot_all_tickers(close_prices):
    ...
    for i, ticker in enumerate(close_prices.columns):
        ...
        if s.empty or len(s) < 2:          # ← add this guard
          continue
...
with tab1:
    ...
    if fc_series.empty or len(series) < 2:
        st.warning(f"⚠️ No data available for {selected_ticker} — likely failed during fetch. Click 'Refresh Live Data' to retry.")
    else:
        last_price  = series.iloc[-1]
        ...
        recent_anom = len(anomaly_data["anomalies"][anomaly_data["anomalies"].index >= cutoff_30]) if anomaly_data else 0
        ...
        with col_anom:
            ...
            if anomaly_data:
                ...
                recent    = anomaly_data["anomalies"][anomaly_data["anomalies"].index >= cutoff_90]
                if recent.empty:
...
with tab2:
    ...
    cutoff_30    = pd.Timestamp.today() - pd.Timedelta(days=30)
    recent_total = sum(len(d["anomalies"][d["anomalies"].index >= cutoff_30]) for d in anomaly_dict.values())
    ...
    for ticker in YF_TICKERS:
        ...
        cutoff_30 = pd.Timestamp.today() - pd.Timedelta(days=30)
        r_anom = len(anom["anomalies"][anom["anomalies"].index >= cutoff_30])
